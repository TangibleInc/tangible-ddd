...
class DDD_Command {
  ...
  public function init( $args, $assoc_args ) {
    ...
    WP_CLI::log( '' );
    WP_CLI::log( '  2. Include the DI container in your plugin bootstrap:' );
    WP_CLI::log( "       require_once __DIR__ . '/ddd-wordpress/di/index.php';" );
    WP_CLI::log( '' );
    WP_CLI::log( '  3. Paste the following block into your main plugin file' );
    WP_CLI::log( "     (after {$version_const} is defined and the autoloader is loaded)." );
    WP_CLI::log( '     This installs framework tables on activation and registers' );
    WP_CLI::log( '     outbox/process hooks on init:' );
    WP_CLI::log( '' );
  ...
  /**
   * Render the post-scaffold wiring snippet — the code block the user pastes
   * into their main plugin file to install tables on activation and register
   * outbox/process hooks on init.
   *
  ...
  private function template_main_plugin_snippet( string $prefix, string $namespace, string $version_const ): string {
    return <<<PHP
// ─── tangible-ddd wiring — generated by `wp ddd init`, copy into your main plugin file ───
register_activation_hook(__FILE__, function (): void {
    \\TangibleDDD\\WordPress\\install_tables(
        new \\{$namespace}\\Infra\\Config({$version_const})
    );
});
...
add_action('init', function (): void {
    \\TangibleDDD\\WordPress\\register_hooks(
        new \\{$namespace}\\Infra\\Config({$version_const}),
        fn() => \\{$namespace}\\WordPress\\DI\\di()
    );
}, 2);
// ─── end tangible-ddd wiring ───
PHP;
  }
